...
**Для русскоязычных пользователей доступно полное руководство по установке и запуску:**

**[🇷🇺 README_RU.md - Установка и Запуск](README_RU.md)** - подробное руководство на русском языке, включающее:
- Пошаговую установку системы
- Настройку окружения (Docker и локальную)
...
**Русскоязычным пользователям рекомендуется начать с:**

**[🇷🇺 README_RU.md - Установка и Запуск](README_RU.md)** - полное руководство на русском языке с пошаговыми инструкциями.
...
**Для развертывания в production окружение см.: [📚 docs/PRODUCTION_DEPLOYMENT.md](docs/PRODUCTION_DEPLOYMENT.md)**

Это руководство включает:
- Подготовку хоста (Ubuntu/Debian)
- Управление секретами и конфигурацией
- Docker Compose развертывание (рекомендуется)
- Bare-metal развертывание (Gunicorn + systemd + Nginx)
- Настройку SSL сертификатов (Let's Encrypt)
- Запуск Celery Worker & Beat сервисов
- Мониторинг и проверки здоровья
- Troubleshooting для типичных production проблем

**Ключевые темы:**
- PostgreSQL и Redis провизионирование
- Backblaze B2 хранилище с Cloudflare CDN
- Email уведомления
