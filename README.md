...
- **HTMX-фронтенд** для клиентских дашбордов и консолей операторов
- **Celery workers и Beat** для оркестрации AI-обработки и периодических задач
- **Redis** для брокера Celery, кэширования подписанных URL и координации фоновых задач
- **PostgreSQL** как основная система хранения (проекты, видео, триггеры, метки, метаданные пайплайна)
...
|-----------|------------|------------|
| **Бэкенд** | Python 3.11, Django 5 | Основное приложение |
| **Фронтенд** | Django Templates, HTMX, Bootstrap 5 | Веб-интерфейс |
| **Очередь задач** | Celery + Redis | Фоновая обработка |
| **База данных** | PostgreSQL (jsonb для AI отчётов) | Основное хранилище |
| **Хранилище** | Backblaze B2 + Cloudflare CDN | Файлы и CDN |
| **AI-инференс** | Replicate (Serverless GPU) | AI-модели |
...
│   ├── users/                  # Управление пользователями (клиенты / операторы / администраторы)
│   ├── projects/               # Проекты и видео
│   ├── ai_pipeline/            # AI-обработка (Celery-задачи и AI-сервисы)
│   ├── operators/              # Рабочий стол оператора
│   ├── admins/                 # Админ-панель
│   └── storage/                # Интеграция с B2/Cloudflare
├── scripts/                    # Вспомогательные скрипты
│   └── entrypoint.sh           # Docker-скрипт точки входа для миграций и collectstatic
├── celery-worker/              # Скрипты Celery worker
├── .env.example                # Пример конфигурации для продакшена
...
- **Git** - `git --version`
- **curl** или **wget** - для тестирования API-эндпоинтов
...
- `BACKBLAZE_*` - учётные данные Backblaze B2
...
# Для продакшеноподобного тестирования (с проверками безопасности)
...
└── prod.py          # Настройки продакшена (строгая валидация, безопасность)
```

### Переменные окружения по доменам

#### Базовые параметры Django
...
| Переменная | Описание | Обязательно |
|------------|----------|------------|
| `BACKBLAZE_ENDPOINT_URL` | Endpoint B2 | Да |
| `BACKBLAZE_APPLICATION_KEY_ID` | Идентификатор ключа приложения | Да |
| `BACKBLAZE_APPLICATION_KEY` | Ключ приложения | Да |
| `BACKBLAZE_BUCKET_NAME` | Имя бакета | Да |

#### Сервисы Replicate AI
...
| Переменная | Описание | Обязательно |
|------------|----------|------------|
| `CLOUDFLARE_ZONE_ID` | Идентификатор зоны Cloudflare (Zone ID) | Нет |
...
#### Настройки B2 и повторных попыток

| Переменная | Описание | По умолчанию |
|------------|----------|-------------|
| `B2_MAX_RETRIES` | Макс. количество попыток B2 | `3` |
| `B2_RETRY_BACKOFF` | Начальный backoff в секундах | `1` |
...
Система предоставляет два взаимодополняющих интерфейса:
...
- **HTMX интерфейс** (`/client/*`) — серверный рендеринг HTML-фрагментов
...
**Время жизни токенов:**
- Токен доступа: 1 час
- Токен обновления: 7 дней

**Использование:**
Включайте токен доступа в заголовке `Authorization` для всех API-запросов:
```http
Authorization: Bearer eyJ0eXAiOiJKV1QiLCJhbGc...
```

#### Сессионная аутентификация (HTMX)

Стандартная аутентификация на базе Django-сессий:
- Вход: `POST /client/users/login/`
- Выход: `POST /client/users/logout/`
- CSRF токен требуется для всех POST/PUT/DELETE запросов
- Сессионный cookie автоматически управляется браузером

### Базовые URL

| Окружение | Базовый URL | Описание |
|-----------|-------------|----------|
| Локальное | `http://localhost:8000` | Разработка |
| Продакшн | `https://yourdomain.com` | Продакшн |
...
HTMX-интерфейс использует те же эндпоинты, но с префиксом `/client/` и возвращает HTML-фрагменты вместо JSON.
...
**Веб-сервис:**
- Команда сборки: `cd backend && python manage.py collectstatic --noinput`
- Команда запуска: `cd backend && gunicorn compliance_app.wsgi:application --bind 0.0.0.0:$PORT`
- Размер инстанса: 1GB RAM (минимум)
- HTTP-порт: 8000

**Сервис-воркер (Celery):**
- Команда запуска: `cd backend && celery -A compliance_app worker --loglevel=info`
- Размер инстанса: 1GB RAM (минимум)

**База данных:**
- PostgreSQL 1GB RAM
- Создайте базу данных и скопируйте строку подключения
...
2. Создайте приватный бакет
3. Создайте ключ приложения (Application Key) с правами на запись
4. Добавьте учётные данные в переменные окружения
...
3. Следите за логами в дашборде DigitalOcean
...
- **DigitalOcean App Platform**: встроенная панель метрик
- **Backblaze B2**: https://secure.backblaze.com/b2_buckets.htm (консоль бакетов)
- **Replicate**: https://replicate.com/account/billing (страница биллинга)
- **Cloudflare**: панель аналитики
...
#### Пользовательская точка проверки состояния
...
### Стоимость DigitalOcean App Platform
...
- ✅ B2 бакеты приватные (подписанные URL)
- ✅ PostgreSQL/Redis с SSL в продакшене
- ✅ Django middleware безопасности включен
